( function () {
	function tablewrap() {
		...
		for ( i = x.length; i--; ) {
			...
			y.className = 'brio-table-wrapper';
			if ( x[ i ].clientWidth > z && x[ i ].parentNode.className !== 'brio-table-wrapper' ) {
				x[ i ].parentNode.insertBefore( y, x[ i ] );
				y.appendChild( x[ i ] );
			} else if ( x[ i ].clientWidth < z && x[ i ].parentNode.className === 'brio-table-wrapper' ) {
